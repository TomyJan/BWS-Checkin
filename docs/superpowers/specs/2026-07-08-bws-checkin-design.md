...
- **鉴权：** 生产环境可配置多个 OAuth provider，当前支持 OIDC 和 QQ OAuth；本地开发使用 mock login。
...
2. 用户点击登录，跳转到已配置的 OAuth provider。
3. OAuth callback 成功后，后端查找关联账号；如果不存在关联账号，则创建站内用户并绑定该 OAuth 账号。
4. 后端写入 HTTP-only Session Cookie。
5. 前端通过 `/api/v1/me` 获取当前用户资料。

用户表保存业务需要的信息：

- 开发登录或 OAuth 登录生成的站内登录 subject
- 显示名称
- 头像地址（如果 OAuth provider 返回）
- 二维码图片路径
- 创建时间和更新时间

生产环境不提供用户名 / 密码注册。只要 OAuth 登录成功，即允许自动注册站内用户。个人中心展示当前用户已绑定的 OAuth 渠道，并允许绑定未绑定渠道；当前不支持解绑。
...
唯一约束：`provider_id + provider_subject`。同一个外部账号只能绑定到一个站内用户。
...
- OAuth 登录创建用户并落库绑定。
- 已登录用户可以在个人中心绑定已配置 OAuth 渠道，不能解绑。
- 创建互助组时 ID 唯一约束生效。
- 加入互助组后能出现在成员列表。
...
- 采用前后端分离架构。
- 后端使用 Go。
- 生产环境使用已配置 OAuth provider 登录，本地开发使用 mock login。
